...
class Discord {
	...
	command(name) {
		return this._commands[Object.keys(this._commands).findIndex(n => n === name || (n[this._args.a] && n[this._args.a].includes(name)))];
	}
	setup() {
		...
		this.client.on("message", msg => {
			...
			const content = msg.content.slice(prefix.length + command);
